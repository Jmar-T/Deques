...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        if (data == null) {
            throw new IllegalArgumentException("Can't add null data to Deque");
        }

        if (size == backingArray.length) {
            T[] tempArray = (T[]) new  Object[backingArray.length * 2];
            for (int i = 0; i < size; i++) {
                tempArray[i + 1] = backingArray[mod(i + front, backingArray.length)];
            }
    ...
    public void addLast(T data) {
        if (data == null) {
            throw new IllegalArgumentException("Can't add null data to a Deque");
        }

        if (size == backingArray.length) {
            T[] tempArray = (T[]) new Object[backingArray.length * 2];
            for (int i = 0; i < size; i++) {
                tempArray[i] = backingArray[mod(i + front, backingArray.length)];
            }
            backingArray = tempArray;
            front = 0;
        }
    ...
    public T removeLast() {
        ...
        T tempData = backingArray[mod(front + size - 1, backingArray.length)];
    ...
    public T getLast() {
        if (size == 0) {
            throw new NoSuchElementException("Can't retrieve from an empty Deque");
        }
